...
static int parseCSVLine(const char* line, char** fields, int maxCols)
{
    int col = 0;
    int charIdx = 0;
    int i = 0;
...
void FileHandler::loadDoctors(Storage<Doctor>& store)
{
    ...
    while (file.getline(line, 1024))
    {
        ...
        {
            Doctor d(Validator::charToInt(*(fields + 0)), *(fields + 1), *(fields + 2), *(fields + 3), *(fields + 4), Validator::charToFloat(*(fields + 5)));
            store.add(d);
        }
    }
...
void FileHandler::loadPrescriptions(Storage<Prescription>& store)
{
    ...
    while (file.getline(line, 1024))
    {
        if (parseCSVLine(line, fields, 7) >= 7)
        {
            Prescription pr(Validator::charToInt(*(fields + 0)), Validator::charToInt(*(fields + 1)), Validator::charToInt(*(fields + 2)), Validator::charToInt(*(fields + 3)), *(fields + 4), *(fields + 5), *(fields + 6));
            store.add(pr);
        }
    }
...
void FileHandler::appendPrescription(const Prescription& pr)
{
    ...
    if (file.is_open())
    {
        file << pr.getId() << "," << pr.getAppointmentId() << "," << pr.getPatientId() << "," << pr.getDoctorId() << "," << pr.getDate() << "," << pr.getMedicines() << "," << pr.getNotes() << "\n";
        file.close();
    }
}
...
void FileHandler::appendSecurityLog(const char* role, const char* enteredId, const char* result)
{
    ...
    strftime(ts, 64, "%d-%m-%Y %H:%M:%S", tinfo);
