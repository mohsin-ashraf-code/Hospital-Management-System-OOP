...
Prescription::Prescription(int i, int aId, int pId, int dId, const char* dt, const char* meds, const char* nts)
    : id(i), appointmentId(aId), patientId(pId), doctorId(dId), date(dt) {
...
const char* Prescription::getDate() const { return date.get(); }
