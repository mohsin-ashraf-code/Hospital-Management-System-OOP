            ...
            else if (i == 2) 
            {
                ...
                for (int j = 0; j < state->patients.getSize(); j++) 
                {
                    ...
                    for (int k = 0; k < state->bills.getSize(); k++) 
                    {
                        ...
                        if (b->getPatientId() == p->getId() && Validator::myStrEqual(b->getStatus(), "unpaid")) 
                        {
                            unpaidCount++;
                        }
                    }
                    display += "ID: " + intToStr(p->getId()) + " | Name: " + String(p->getName()) + " | Bal: " + intToStr((int)p->getBalance()) + " | Unpaid: " + intToStr(unpaidCount) + "\n";
                }
                dataViewer.show(display);
            }
            else if (i == 3) 
            {
                String display = "ALL DOCTORS\n";
                for (int j = 0; j < state->doctors.getSize(); j++) 
                {
                    Doctor* d = state->doctors.getAt(j);
                    display += "ID: " + intToStr(d->getId()) + " | Dr. " + String(d->getName()) + " | Spec: " + String(d->getSpecialization()) + " | Fee: " + intToStr((int)d->getFee()) + "\n";
                }
                dataViewer.show(display);
            }
            else if (i == 4) 
            {
                String display = "ALL APPOINTMENTS\n";
                for (int j = 0; j < state->appointments.getSize(); j++) 
                {
                    Appointment* a = state->appointments.getAt(j);
                    display += "ID: " + intToStr(a->getId()) + " | P_ID: " + intToStr(a->getPatientId()) + " | D_ID: " + intToStr(a->getDoctorId()) + " | Date: " + String(a->getDate()) + " | Status: " + String(a->getStatus()) + "\n";
                }
                dataViewer.show(display);
            }
            else if (i == 5) 
            {
                String display = "UNPAID BILLS\n";
                for (int j = 0; j < state->bills.getSize(); j++) 
                {
                    Bill* b = state->bills.getAt(j);
                    if (Validator::myStrEqual(b->getStatus(), "unpaid")) 
                    {
                        display += "Bill ID: " + intToStr(b->getId()) + " | Amt: " + intToStr((int)b->getAmount()) + " | Date: " + String(b->getDate());
            ...
            else if (i == 8) 
            {
                ...
                for (int j = 0; j < state->appointments.getSize(); j++) 
                {
                    ...
                    if (Validator::myStrEqual(a->getDate(), timeBuf)) 
                    {
                        ...
                        if (Validator::myStrEqual(s, "pending"))
                        {
                            pending++;
                        }
                        else if (Validator::myStrEqual(s, "completed"))
                        {
                            comp++;
                        }
                        else if (Validator::myStrEqual(s, "noshow"))
                        {
                            noshow++;
                        }
                        else if (Validator::myStrEqual(s, "cancelled"))
                        {
                            canc++;
                        }
                    }
                ...
                String display = "DAILY REPORT (" + String(timeBuf) + ")\n";
...
void AdminScreen::update(float dt, RenderWindow& window) 
{
    ...
    else if (dischargeForm.isActive())
    {
        dischargeForm.update(window);
    } 
    else 
    {
        for (int i = 0; i < 10; i++)
